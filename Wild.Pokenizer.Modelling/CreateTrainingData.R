...
for (dir in sourceDirs)
{
  ...
  message("Creating directories for ",
          dir,
          " and copying files from ",
          source_dir)
  ...
  number_of_files_in_training_set <-
    training_set_size * length(files)
  number_of_files_in_validation_set <-
    validation_set_size * length(files)
  ...
  train_sample_indices <-
    sample(1:length(files), number_of_files_in_training_set)
  ...
  if (number_of_files_in_test_set > 0)
  {
    ...
    validation_sample_indices <-
      sample(1:length(files), number_of_files_in_validation_set)
  ...
  file.copy(file.path(source_dir, train_sample_set),
            file.path(train_dir))
  file.copy(file.path(source_dir, validation_sample_set),
            file.path(val_dir))
  if (number_of_files_in_test_set > 0)
  {
    file.copy(file.path(source_dir, test_sample_set),
              file.path(test_dir))
  }
}
...
df$filepart <-
  sapply(strsplit(as.character(df$fname), '.jpg'), "[", 1)
df$label <- sapply(strsplit(as.character(df$filepart), '_'), "[", 1)
df$number <-
  as.numeric(sapply(strsplit(as.character(df$filepart), '_'), "[", 2))
